...
if __name__ == '__main__':
    ...
    # Вибір зображення цифри для навчання під довільним індексом та вивід на екран його числових даних
    train_image_index = randint(0, len(train_images) - 1)
    digit = train_images[train_image_index]
    print(f">>> Was chosen train image with index {train_image_index}\n. The digit is:\n")
    ...
    # Вибір тестового довільного зображення
    test_image_index = randint(0, len(test_images) - 1)
    print(f">>> Showing the selected test image with index: {test_image_index}")
    plt.imshow(test_images[test_image_index], cmap='binary')
    plt.savefig("Test_image_digit.png")
    plt.close()

    # Прогноз на обробленому зображенні
    print(">>> Predicting the class of the image")
    test_image_expand = expand_dims(test_images_reshaped[test_image_index], 0)
    print("Network prediction result: ", argmax(network.predict(test_image_expand)))
    print("Correct answer: ", test_labels[test_image_index].argmax())
